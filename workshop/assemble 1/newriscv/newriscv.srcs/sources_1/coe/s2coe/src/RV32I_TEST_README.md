...
### LED 输出地址
程序将测试结果写入到特定内存地址供 LED 显示：

**主输出地址: `0xFFFFFFFC`**
- 如果所有测试通过：输出 `0x22` (十进制 34)
- 如果测试失败：输出 `0xFFFFFFFF` (全 1)

**调试输出地址: `0x2000`**
- 偏移 +0: 通过的测试数量
- 偏移 +4: 完成标记 `0xDEADBEEF`
...
2. **连接 LED**
   - 将地址 `0xFFFFFFFC` 映射到 GPIO/LED 输出端口
   - LED 应显示二进制值

3. **预期结果**
   ```
   LED 显示: 0x22 (二进制: 00100010)
   十进制: 34
   表示: 34 个测试全部通过
   ```

### LED 位含义（8-bit LED 显示）
```
Bit 7-6: 测试完成（应该为 00）
Bit 5-0: 通过的测试数量（应该为 34 = 0b100010）
```

如果看到 `0xFF`（所有 LED 亮），表示测试失败。
...
### 仿真验证
在仿真器中运行时：
1. 设置断点在 `test_complete` 标签
2. 检查寄存器 `x31` 的值（应为 34）
3. 检查内存地址 `0x2000` 的内容

### 单步测试
每个测试失败时会跳转到 `fail_halt`：
- 可以在仿真中观察哪个测试失败
- 检查 `x31` 的值，知道在哪个测试失败
